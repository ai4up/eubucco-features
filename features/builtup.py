...
#     mapping = _reverse(GHS_CAT_AVG_HEIGHTS)
#     mean_raster = raster_agg.aggregate_raster(raster, meta, buffer_m, mapping, agg="mean")
#     print('##### max mean_raster val:', np.nanmax(mean_raster['array']))
#     print('##### min mean_raster val:', np.nanmin(mean_raster['array']))
#     points = buildings.to_crs(mean_raster["crs"]).centroid

#     return raster_agg.sample_raster_at_points(mean_raster, points)


# def ghs_type_shares(buildings, raster, meta, buffer_m):
#     mapping = defaultdict(lambda: 0, {11: 1, 12: 1, 13: 1, 14: 1, 15: 1,
#                                         21: 2, 22: 2, 23: 2, 24: 2, 25: 2})
#     class_values = np.array(list(GHS_USE_TYPES.values())).flatten()
#     shares = raster_agg.aggregate_raster(raster, meta, buffer_m, mapping, agg="share", preproc_default=0, classes=[1, 2])
#     print(shares)
#     points = buildings.to_crs(next(iter(shares.values()))["crs"]).centroid
#     return {cls: raster_agg.sample_raster_at_points(r, points)
#             for cls, r in shares.items()}
