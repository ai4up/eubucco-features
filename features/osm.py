...
def closest_building_attributes(buildings: gpd.GeoDataFrame, osm_buildings: gpd.GeoDataFrame, attributes: Union[List[str], Dict[str, str]]) -> gpd.GeoDataFrame:
...
def distance_to_some_building_type(buildings: gpd.GeoDataFrame, osm_buildings: gpd.GeoDataFrame, use_type: str) -> pd.Series:
    osm_buildings = osm_buildings[osm_buildings['type'] == use_type]
    dis = buildings.centroid.distance(osm_buildings.geometry.union_all())
    return dis


def distance_to_some_building_height(buildings: gpd.GeoDataFrame, osm_buildings: gpd.GeoDataFrame, height_range: Tuple[float, float]) -> pd.Series:
    osm_buildings = osm_buildings[osm_buildings['height'].between(*height_range)]
    dis = buildings.centroid.distance(osm_buildings.geometry.union_all())
    return dis


def building_type_share_buffer(osm_buildings: gpd.GeoDataFrame, h3_res: int, k: Union[int, List[int]]) -> gpd.GeoDataFrame:
