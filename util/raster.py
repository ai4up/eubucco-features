...
def read_values(filepath: str, points: gpd.GeoSeries) -> pd.Series:
    with rasterio.open(filepath) as src:
        ...
        rows, cols = _geom_to_rowcol(points, src.transform, src.crs)
...
# def sum_pooling(raster_data, block_size=5):
#     stride = 1
#     pad_size = block_size - stride

#     padded_data = np.pad(raster_data, pad_width=((pad_size, 0), (pad_size, 0)), mode='constant', constant_values=0)
#     pooled = sliding_window_view(padded_data, (block_size, block_size)).sum(axis=(-1, -2))
...
# def share_pooling(raster_data, block_size=5, category: int):
#     mask = np.int8(raster_data == category)
#     pooled = uniform_filter(mask, size=block_size)
...
def _nanmean_pooling(raster_data: np.ndarray, size: int) -> np.ndarray:
    ...
    # Mean ignoring NaNs
    # mean = np.where(count > 0, summed / count, np.nan)
